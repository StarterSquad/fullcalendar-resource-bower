...
if(M){M=parseInt(M[1])*I("slotMinutes");var ja=Math.floor(M/60);w.setHours(ja);w.setMinutes(M%60+La);H("dayClick",g[j],w,false,c,R[j])}else H("dayClick",g[j],w,true,c,R[j])}}function la(c,j,w,M){w&&Ia.build();c=ka(M);F(ma(0,c,0,c+1-1))}function ma(c,j,w,M){c=Ia.rect(c,j,w,M,X);return x(c,X)}function wa(c,j){for(var w=J(n.visStart),M=ha(J(w),1),ja=0;ja<Ha;ja++){var ya=new Date(Math.max(w,c)),Da=new Date(Math.min(M,j));if(ya<Da){var Ea=ja*Ka+Na;Ea=Ia.rect(0,Ea,0,Ea,K);ya=oa(w,ya);Da=oa(w,Da);Ea.top=
ya;Ea.height=Da-ya;p(x(Ea,K))}ha(w,1);ha(M,1)}}function za(c){return bb.left(c)}function pa(c){return bb.right(c)}function na(c){return{row:Math.floor(Ja(c,n.visStart)/7),col:$(c.getDay())}}function qa(c){var j=Aa(c.col);c=c.row;I("allDaySlot")&&c--;c>=0&&ua(j,La+c*I("slotMinutes"));return j}function Aa(){return J(n.visStart)}function sa(c){return I("allDaySlot")&&!c.row}function $(){return 0}function ka(c){for(var j=0;j<R.length;j++)if(c.id===R[j].id)return j;return-1}function oa(c,j){c=J(c,true);
if(j<ua(J(c),La))return 0;if(j>=ua(J(c),eb))return ba.height();c=I("slotMinutes");j=j.getHours()*60+j.getMinutes()-La;var w=Math.floor(j/c),M=Va[w];if(M===ra)M=Va[w]=ba.find("tr:eq("+w+") td div")[0].offsetTop;return Math.max(0,Math.round(M-1+ab*(j%c/c)))}function ta(){return{left:Ma,right:ob-Wa}}function Q(){return C}function E(c){var j=J(c.start);if(c.allDay)return j;return ua(j,I("defaultEventMinutes"))}function T(c,j){if(j)return J(c);return ua(J(c),I("slotMinutes"))}function d(c,j,w,M){if(w)I("allDaySlot")&&
